...
#### Scenario: 代理渲染还原背景模糊填充
- **WHEN** 竖屏视频片段通过 `extra_material_refs` 引用合法的 `canvas_blur` 素材
- **THEN** `render proxy` 在横屏画布空白区域生成由该视频帧衍生的模糊背景，同时保留前景构图；不得继续输出纯黑补边或把背景素材误用于其他片段
- **AND** `canvas_blur` 缺失、无效或引用冲突时必须失败关闭；代理的模糊强度只能作为预览近似，不能冒充真实剪映原生导出一致性
